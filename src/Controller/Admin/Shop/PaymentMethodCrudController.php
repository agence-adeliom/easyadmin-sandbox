...
class PaymentMethodCrudController extends AbstractCrudController
{
    ...
    public function __construct(AdminUrlGenerator $crudUrlGenerator, ParameterBagInterface $parameterBag, PaymentMethodFactoryInterface $paymentMethodFactory)
    {
    ...
    public function configureActions(Actions $actions): Actions
    {
        ...
        foreach ($this->parameterBag->get('sylius.payment_gateways') as $gatewayFactoryName => $gatewayFactory) {
            $newAdd = Action::new($gatewayFactoryName, 'Créer ' . $gatewayFactoryName)->linkToUrl((clone $url)->set("gatewayFactory", $gatewayFactory)->set("gatewayFactoryName", $gatewayFactoryName)->generateUrl())->createAsGlobalAction()->setCssClass("btn btn-primary");
    ...
    {
        global $gatewayFactory, $gatewayFactoryName;
        $syliusPaymentMethod = $this->paymentMethodFactory->createWithGateway($gatewayFactory);
