...
"use strict";
/*!*****************************************************!*\
  !*** ./assets/js/form-type-association-new-ajax.js ***!
  \*****************************************************/
__webpack_require__.r(__webpack_exports__);
...
var AddNewProperty = {
  ...
  setModalContent: function setModalContent(widget, data) {
    var field = widget.find('[data-ea-ajax-new-endpoint-url]');
    var url = field.data('ea-ajax-new-endpoint-url');
    var html = $(data);
    var title = html.find('.content-header-title .title').html();
    var form = html.find('.ea-new-form');
    var footer = html.find('.page-actions');
    footer.find('[value="saveAndAddAnother"]').remove();
    var modal = widget.find('.create-entity-modal');
    ...
    form.on('submit', function (e) {
      e.preventDefault();
      var jForm = $(this);
      var data = AddNewProperty.serializeForm(jForm);
      ...
      $.ajax({
        ...
        processData: false,
        headers: {
          'X-Requested-With': 'AssociationField'
        },
        success: function success(result, status, xhr) {
          var id = xhr.getResponseHeader("x-crud-entity-id");
          var name = xhr.getResponseHeader("x-crud-entity-name");

          if (id && name) {
            var _instance$addOption;

            var instance = field[0].tomselect;
            instance.addOption((_instance$addOption = {}, _defineProperty(_instance$addOption, instance.settings.valueField, id), _defineProperty(_instance$addOption, instance.settings.labelField, name), _instance$addOption));
            instance.refreshOptions(false);
            instance.addItem(id);
            instance.refreshItems();
            modal[0].bsModal.hide();
          } else AddNewProperty.setModalContent(widget, result);
        }
      });
    });
  },
  handleAddClickButton: function handleAddClickButton(addButton) {
    var parent = addButton.parents(".form-widget");
    var modal = parent.find('.create-entity-modal');
    modal[0].bsModal = new bootstrap_js_src_modal__WEBPACK_IMPORTED_MODULE_1__.default(modal[0]);
    var field = parent.find('[data-ea-ajax-new-endpoint-url]');
    var url = field.data('ea-ajax-new-endpoint-url');
    var iconElement = addButton.find('.fa');
