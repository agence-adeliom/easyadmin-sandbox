...
"use strict";
/*!*************************************************!*\
  !*** ./assets/js/form-type-association-list.js ***!
  \*************************************************/
__webpack_require__.r(__webpack_exports__);
...
/* harmony import */ var bootstrap_js_src_modal__WEBPACK_IMPORTED_MODULE_19__ = __webpack_require__(/*! bootstrap/js/src/modal */ "./node_modules/bootstrap/js/src/modal.js");
...
window.addEventListener('DOMContentLoaded', function (event) {
  ...
  var modalBody = '.create-entity-modal .content-body';
  $('body').on('click', "".concat(modalBody, " a"), function (e) {
    AddListProperty.handleInnerLink(e, $(this));
  });
  $('body').on('submit', "".concat(modalBody, " form"), function (e) {
    AddListProperty.handleInnerForm(e, $(this));
  });
  $('body').on('click', "".concat(modalBody, " tr"), function (e) {
    e.preventDefault();
...
var AddListProperty = {
  ...
  getRowName: function getRowName(jRow) {
    if (AddListProperty.columns && Array.isArray(AddListProperty.columns.columns)) {
      var values = [];
      var nbCol = AddListProperty.columns.columns.length;

      for (var i = 0; i < nbCol; i++) {
        var col = AddListProperty.columns.columns[i];
        var val = jRow.find("td:nth-child(".concat(col, ")")).text().trim();
      ...
      return values.join(" ".concat(AddListProperty.columns.separator ? AddListProperty.columns.separator : '-', " "));
    }
  ...
  selectValues: function selectValues() {
    if (AddListProperty.selection.length) {
      ...
      for (var i in AddListProperty.selection) {
        var id = AddListProperty.selection[i];
        var name = AddListProperty.getRowName(AddListProperty.modal.find("[data-id=\"".concat(id, "\"]")));
        var value = {
          text: name,
          id: id
        };
        ...
        if ('ea-autocomplete' === AddListProperty.field.data('ea-widget')) {
          var _instance$addOption;

          var instance = AddListProperty.field[0].tomselect;
          instance.addOption((_instance$addOption = {}, _defineProperty(_instance$addOption, instance.settings.valueField, value.id), _defineProperty(_instance$addOption, instance.settings.labelField, value.text ? value.text : "#" + value.id), _instance$addOption));
        ...
        if ('select2' === AddListProperty.field.data('widget')) {
          ...
          if (!AddListProperty.field.find("option[value=\"".concat(value.id, "\"]")).length) {
  ...
  handleRowSelection: function handleRowSelection(row) {
    AddListProperty.selectRow(row);
  },
  buildFooter: function buildFooter(listButton) {
    var cancelLabel = listButton.data('cancel-label');
    var validateLabel = listButton.data('validate-label');
    var buttons = [{
      class: 'btn-secondary',
      data: 'js-list-cancel',
      icon: '',
    ...
    for (var i in buttons) {
      ...
      html += "<button class=\"btn ".concat(button.class, "\" ").concat(button.data, ">") + "<span class=\"btn-label\">" + "<i class=\"action-icon fa ".concat(button.icon, "\"></i> ").concat(button.label, "</span>") + "</button>";
    }
  ...
  setInnerHTML: function setInnerHTML(element, htmlContent) {
  ...
  handleInnerLink: function handleInnerLink(e, link) {
    var destUrl = link.attr('href');
    var isModal = link.data('modal');

    if (!isModal) {
      e.preventDefault();
      $.get(destUrl, function (data) {
        var html = $(data);
        var body = html.find('.content-body');
        AddListProperty.populateModalBody(body);
      });
    } else {
      var filterButton = e.currentTarget;
      var filterModal = document.querySelector(filterButton.dataset.modal);
      ...
      $.get(filterButton.getAttribute('href'), function (response) {
        ...
        var header = $(filterModal).find('.modal-header').html();
        ...
        $('#modal-apply-button').on('click', function (e) {
          e.preventDefault();
          e.stopPropagation();
          filterModal.querySelectorAll('.filter-checkbox:not(:checked)').forEach(function (notAppliedFilter) {
        ...
        $('#modal-clear-button').on('click', function (e) {
          e.preventDefault();
          e.stopPropagation();
          filterModal.querySelectorAll('.filter-field').forEach(function (filterField) {
            filterField.remove();
          });
          $(filterModalBody).find('form').submit();
        });
  ...
  handleInnerForm: function handleInnerForm(e, form) {
    e.preventDefault();
    e.stopPropagation();
    var method = form.attr('method');
    var query = new URLSearchParams(new FormData(form[0])).toString();
    $.ajax({
  ...
  addSelectors: function addSelectors(html) {
    var rows = html.find('tr');
    rows.each(function () {
      var row = $(this);
      var actionsCell = row.find('.actions');
      var checkbox = $('<input type="checkbox" />').css('pointer-events', 'none');
      actionsCell.html(checkbox);
      row.css('cursor', 'pointer');
    });
  },
  manageSelectionDisplay: function manageSelectionDisplay() {
    var rows = AddListProperty.modal.find('tr');
    ...
    if (AddListProperty.selection.length) {
      for (var i in AddListProperty.selection) {
        var id = AddListProperty.selection[i];
        var row = AddListProperty.modal.find("[data-id=\"".concat(id, "\"]"));
    ...
    for (p in obj) {
      if (obj.hasOwnProperty(p)) {
        var k = prefix ? prefix + "[" + p + "]" : p,
            v = obj[p];
        str.push(v !== null && _typeof(v) === "object" ? AddListProperty.serialize(v, k) : encodeURIComponent(k) + "=" + encodeURIComponent(v));
      }
    }

    return str.join("&");
  },
  handleListClickButton: function handleListClickButton(listButton) {
    var iconElement = listButton.find('.fa');
    var originalIcon = iconElement.attr('class');
    iconElement.removeClass();
    iconElement.addClass('fa fa-spinner fa-spin pr-1');
    listButton.attr('disabled', true);
    var url = AddListProperty.url;
    var filters = listButton.data('filters');
    var query;

    if (filters) {
      query = AddListProperty.serialize({
        filters: filters
      });
      url += '&' + query;
    }
    ...
    if (url) {
      $.get(url, function (data) {
        var html = $(data);
        var title = html.find('.content-header-title .title').html();
        var body = html.find('.content-body');
        var footer = AddListProperty.buildFooter(listButton);
  ...
  populateModalBody: function populateModalBody(html) {
    var options = [{
      data: 'show-filter',
    ...
    for (var i in options) {
      ...
      var showOption = AddListProperty.button.data(option.data);
  ...
  init: function init(listButton) {
    var parent = listButton.parents(".form-widget");
    AddListProperty.button = listButton;
    AddListProperty.field = parent.find("[data-".concat(AddListProperty.dataProps, "]"));
    AddListProperty.modal = parent.find('.create-entity-modal');
    AddListProperty.modalToggle = new bootstrap_js_src_modal__WEBPACK_IMPORTED_MODULE_19__.default(AddListProperty.modal[0]);
    AddListProperty.url = AddListProperty.field.data(AddListProperty.dataProps);
    AddListProperty.columns = listButton.data('columns');
    var val = AddListProperty.field.val();
    AddListProperty.selection = [];
