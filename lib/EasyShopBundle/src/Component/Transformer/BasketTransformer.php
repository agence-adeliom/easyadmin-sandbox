...
use Adeliom\EasyShop\Component\Payment\PaymentInterface;
...
class BasketTransformer extends BaseTransformer
{
     ...
     *
     * @param \Adeliom\EasyShop\Component\Basket\BasketInterface|null $basket
     *
     * @throws \RuntimeException
     *
     * @return \Adeliom\EasyShop\Component\Order\OrderInterface|null
     */
    public function transformIntoOrder(BasketInterface $basket)
    {
        ...
        if (!$customer instanceof CustomerInterface) {
            if ($this->getLogger()) {
                $this->getLogger()->emergency('[Adeliom\EasyShop\Component\Payment\Transform\Basket::transform] the customer is not valid');
            }
        ...
        if (!$billingAddress instanceof AddressInterface) {
            if ($this->getLogger()) {
                $this->getLogger()->emergency('[Adeliom\EasyShop\Component\Payment\Transform\Basket::transform] the billing address is not valid');
            }
        ...
        if (!$paymentMethod instanceof PaymentInterface) {
            if ($this->getLogger()) {
                $this->getLogger()->emergency('[Adeliom\EasyShop\Component\Payment\PaymentInterface::transform] the payment method is not valid');
            }
        ...
        if (!$deliveryMethod instanceof ServiceDeliveryInterface) {
            if ($this->getLogger()) {
                $this->getLogger()->emergency('[Adeliom\EasyShop\Component\Delivery\ServiceDeliveryInterface::transform] the delivery method is not valid');
            }
        ...
        if ($deliveryMethod->isAddressRequired() && !$deliveryAddress instanceof AddressInterface) {
            if ($this->getLogger()) {
                $this->getLogger()->emergency('[Adeliom\EasyShop\Component\Delivery\ServiceDeliveryInterface::transform] the shipping address is not valid');
            }
