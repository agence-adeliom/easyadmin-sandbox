...
trait GetContent
{
    ...
    protected function getFolderContent($folder, $rec = false)
    {
        ...
        $contents = $this->filesystem->listContents($folder, $rec)->toArray();

        /*$keys = ['mimetype', 'visibility', 'timestamp', 'size'];
        foreach ($contents as $index => $object) {
            dump($object);
            exit;
            if ($object->type === 'file') {
                $missingKeys = array_diff($keys, array_keys((array) $object));
                $contents[$index] = array_reduce($missingKeys, [$this, 'getMetadataByName'], (array) $object);
            }
        }*/
